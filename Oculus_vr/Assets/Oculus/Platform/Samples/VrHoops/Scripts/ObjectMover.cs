...
public class ObjectMover : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        float up = Input.GetKey(KeyCode.UpArrow) ? 1.0f : 0.0f;
        float down = Input.GetKey(KeyCode.DownArrow) ? 1.0f : 0.0f;

        if (Input.GetKeyDown(KeyCode.O))
        {
            movableObject.transform.position = new Vector3(movableObject.transform.position.x, movableObject.transform.position.y + moveSpeed2, movableObject.transform.position.z);
        }
